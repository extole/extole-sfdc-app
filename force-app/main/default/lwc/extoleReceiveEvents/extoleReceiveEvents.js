...
export default class ExtoleReceiveEvents extends LightningElement {
    ...
    async connectedCallback() {
        try { this.userTimezone = await getCurrentUserTimezone(); } catch (e) {}
        await Promise.all([this.loadEndpoint(), this.loadConfigs(), this.loadLogs()]);
    }
    ...
    decorateConfig(c) {
        ...
        return {
            ...c,
            mappingCount: mappingCount === 0 ? '—' : `${mappingCount} field${mappingCount === 1 ? '' : 's'}`,
            isActive:   c.Status__c === 'ACTIVE',
            isInactive: c.Status__c === 'INACTIVE',
            statusBadgeClass: c.Status__c === 'ACTIVE'
    ...
    decorateLog(l) {
        ...
        return {
            ...
            shortRecordId: l.Record_Id__c ? l.Record_Id__c.substring(0, 15) : '—',
            errorPreview: l.Error__c ? l.Error__c.substring(0, 60) + (l.Error__c.length > 60 ? '…' : '') : '',
    ...
    get modalTitle() {
        return this.editingConfig && this.editingConfig.Id ? 'Edit Writeback Rule' : 'New Writeback Rule';
    }
    ...
    handleCopyEndpoint() {
        navigator.clipboard.writeText(this.endpointUrl).then(() => {
            ...
            this.showError('Could not copy — select and copy manually.', null);
        });
    }
    ...
    handleRefreshLogs()    { this.loadLogs(); }
    ...
    async setStatus(cfg, newStatus) {
        try {
            await saveWritebackConfig({ config: { ...cfg, Status__c: newStatus } });
            await this.loadConfigs();
            this.showSuccess(`Rule ${newStatus === 'ACTIVE' ? 'activated' : 'deactivated'}.`);
        } catch (error) {
    ...
    async handleDelete(cfg) {
        if (!confirm(`Delete writeback rule for "${cfg.Event_Name__c}" → ${cfg.Object_Type__c}? This cannot be undone.`)) return;
        try {
    ...
    handleAddMapping() {
        this.mappingRows = [...this.mappingRows, { rowId: newRowId(), extoleKey: '', sfdcField: '' }];
    }
    ...
    async handleSaveModal() {
        ...
        const validRows = this.mappingRows.filter(r => r.extoleKey.trim() && r.sfdcField.trim());
        const mappingsJson = validRows.length > 0
            ? JSON.stringify(validRows.map(r => ({ extoleKey: r.extoleKey.trim(), sfdcField: r.sfdcField.trim() })))
            : null;
        ...
        try {
            ...
            this.modalError = (error && error.body && error.body.message) || 'Failed to save rule.';
        } finally {
            this.isSaving = false;
        }
    }
    ...
    async handleConfirmClearLogs() {
        ...
        try {
            ...
            if (remaining > 0) {
                ...
                this.showSuccess(`Cleared ${deleted} event log ${deleted === 1 ? 'entry' : 'entries'}.`);
            }
        } catch (error) {
